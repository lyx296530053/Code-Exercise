...
<div class="container" style="width: 400px;">
    ...
    <form action="${pageContext.request.contextPath}/update?id=${pageContext.request.getParameter("id")}" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="age" value="${user.getAge()}" name="age" placeholder="请输入年龄"/>
        </div>

        <div class="form-group">
            <%--@declare id="address"--%><label for="address">籍贯：</label>
            <select name="address" class="form-control">
                <c:if test="${user.getAddress()=='广东'}">
                <option value="广东" selected>广东</option>
                <option value="广西">广西</option>
                <option value="湖南">湖南</option>
                </c:if>
                <c:if test="${user.getAddress()=='广西'}">
                    <option value="广东">广东</option>
                    <option value="广西"selected>广西</option>
                    <option value="湖南">湖南</option>
                </c:if>
                <c:if test="${user.getAddress()=='湖南'}">
                    <option value="广东">广东</option>
                    <option value="广西">广西</option>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="${user.getQq()}" name="qq" placeholder="请输入QQ号码"/>
        </div>

        <div class="form-group">
            <%--@declare id="emile"--%><label for="emile">Email：</label>
            <input type="text" class="form-control" value="${user.getEmile()}" name="emile" placeholder="请输入邮箱地址"/>
        </div>
